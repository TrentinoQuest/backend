...
export async function checkIn(
  ...
  const basePoints = computePointsAwarded(quest);
  ...
  // Unica fonte di accredito valuta: applyGamification calcola le monete
  // come basePoints x moltiplicatore streak e le accredita atomicamente.
  const applied = await applyGamification(String(playerId), QuestType.SECONDARY, basePoints);

  logger.info(
    {
      playerId: String(playerId),
      questId: String(quest._id),
      coinsAwarded: applied.gamification.coinsAwarded,
      xpAwarded: applied.gamification.xpAwarded,
  ...
  return {
    completion,
    pointsAwarded: applied.gamification.coinsAwarded,
    totalPoints: applied.totalPoints,
    distanceFromTargetMeters: Math.round(distance),
    gamification: applied.gamification,
  };
}
...
export async function scanQr(
  ...
  const basePoints = computePointsAwarded(questByToken);
  const completion = await createCompletion({
    playerId,
  ...
  // Unica fonte di accredito valuta: applyGamification calcola le monete
  // come basePoints x moltiplicatore streak e le accredita atomicamente.
  const applied = await applyGamification(String(playerId), QuestType.PRIMARY, basePoints);
...
/**
 * Calcola i punti base da registrare sul completamento.
 *
 * I punti base della quest sono il valore storico registrato sul
 * Completion; la valuta effettivamente accreditata al giocatore e'
 * calcolata da applyGamification (basePoints x moltiplicatore streak).
 * Il moltiplicatore zona richiesto dal RF22 ("punti bonus per quest in
 * zone meno visitate") verra' implementato quando avremo dati statistici
 * di affluenza, ovvero dopo l'integrazione del modulo analytics.
 */
