...
describe('sendKudos', () => {
  ...
  it('crea un documento Kudos nel DB', async () => {
    ...
    await sendKudos(from.id, to.id, 'quest_completion', 'activity-xyz', 'star');
    const doc = await Kudos.findOne({ fromPlayerId: from.id, activityId: 'activity-xyz' });
  ...
  it('lancia ConflictError KUDOS_ALREADY_SENT per kudos duplicato sulla stessa attività', async () => {
    ...
    await sendKudos(from.id, to.id, 'quest_completion', 'dup-activity', 'beer');
    await expect(
      sendKudos(from.id, to.id, 'quest_completion', 'dup-activity', 'highfive'),
  ...
  it('permette kudos diversi sulla stessa attività da player diversi', async () => {
    ...
    await sendKudos(from1.id, to.id, 'quest_completion', 'shared-act', 'beer');
    await expect(
      sendKudos(from2.id, to.id, 'quest_completion', 'shared-act', 'star'),
  ...
  it('permette lo stesso player di mandare kudos su attività diverse', async () => {
    ...
    await sendKudos(from.id, to.id, 'quest_completion', 'act-first', 'beer');
    await expect(
      sendKudos(from.id, to.id, 'quest_completion', 'act-second', 'beer'),
    ).resolves.not.toThrow();
  });
});
...
describe('sendFriendRequest', () => {
  it('lancia BadRequestError CANNOT_FRIEND_SELF se si aggiunge se stessi', async () => {
    ...
    await expect(sendFriendRequest(player.id, player.username)).rejects.toMatchObject({
  ...
  it('crea una richiesta di amicizia pending', async () => {
    ...
    await sendFriendRequest(req.id, rec.username);
  ...
  it('lancia ConflictError FRIENDSHIP_ALREADY_EXISTS per richiesta duplicata', async () => {
    ...
    await sendFriendRequest(req.id, rec.username);
    await expect(sendFriendRequest(req.id, rec.username)).rejects.toMatchObject({
  ...
  it('lancia ConflictError FRIENDSHIP_ALREADY_EXISTS se la richiesta inversa esiste già', async () => {
    ...
    await sendFriendRequest(p1.id, p2.username);
    await expect(sendFriendRequest(p2.id, p1.username)).rejects.toMatchObject({
      code: 'FRIENDSHIP_ALREADY_EXISTS',
    });
  });
});
...
describe('acceptFriendRequest', () => {
  it('accetta la richiesta di amicizia e aggiorna status a accepted', async () => {
    ...
    await sendFriendRequest(requester.id, recipient.username);
...
describe('getPendingRequests', () => {
  ...
  it('non include richieste inviate (solo ricevute)', async () => {
    ...
    await sendFriendRequest(sender.id, receiver.username);
