...
import {
  applyGamification,
  awardXpAndCoins,
} from '../../src/modules/quests/services/gamification.service';
...
describe('applyGamification — coinsAwarded', () => {
  it('assegna 10 monete per quest secondaria con streak 0', async () => {
    ...
    const { gamification: result } = await applyGamification(playerId, QuestType.SECONDARY, 10);
  ...
  it('assegna 25 monete per quest principale con streak 0', async () => {
    ...
    const { gamification: result } = await applyGamification(playerId, QuestType.PRIMARY, 25);
...
describe('applyGamification — weeklyXp lega', () => {
  ...
  it('non lancia errore se non esiste stagione attiva', async () => {
    await expect(applyGamification(playerId, QuestType.SECONDARY, 10)).resolves.not.toThrow();
  });
});
...
describe('awardXpAndCoins', () => {
  it('accredita xp e monete e ritorna i totali aggiornati', async () => {
    const playerId = await createTestPlayer('x1');
    const result = await awardXpAndCoins(playerId, 100, 50);
  ...
  it('ricalcola il livello quando gli xp superano la soglia', async () => {
    const playerId = await createTestPlayer('x2');
    // Livello 2 richiede 200 XP
    const result = await awardXpAndCoins(playerId, 250, 0);
  ...
  it('aggiorna weeklyXp della lega se esiste una stagione attiva', async () => {
    const playerId = await createTestPlayer('x3');
    const season = await LeagueSeason.create({
      weekStart: new Date(),
      weekEnd: new Date(Date.now() + 7 * 86400000),
      active: true,
    ...
    await LeagueMembership.create({
      playerId,
      seasonId: season._id,
      groupId: 'test-group-award',
      tier: LeagueTier.PORFIDO,
      weeklyXp: 0,
    });

    await awardXpAndCoins(playerId, 40, 20);

    const membership = await LeagueMembership.findOne({ playerId, seasonId: season._id });
    expect(membership?.weeklyXp).toBe(40);
  });

  it('lancia PLAYER_NOT_FOUND per id inesistente', async () => {
    await expect(awardXpAndCoins('64b000000000000000000000', 10, 10)).rejects.toMatchObject({
      code: 'PLAYER_NOT_FOUND',
    });
  });
});

// ── Fonte unica della valuta ─────────────────────────────────────────────────

describe('applyGamification — fonte unica valuta', () => {
  it('coinsAwarded = basePoints x moltiplicatore streak', async () => {
    const playerId = await createTestPlayer('y1');
    const { gamification } = await applyGamification(playerId, QuestType.SECONDARY, 50);
    // streak 1 → moltiplicatore 1.0
    expect(gamification.coinsAwarded).toBe(50);
  });

  it('il saldo ritornato coincide con quello persistito', async () => {
    const playerId = await createTestPlayer('y2');
    await Player.findByIdAndUpdate(playerId, { totalPoints: 30 });

    const applied = await applyGamification(playerId, QuestType.PRIMARY, 80);

    const player = await Player.findById(playerId);
    expect(applied.totalPoints).toBe(player?.totalPoints);
    expect(applied.totalPoints).toBe(30 + applied.gamification.coinsAwarded);
  });
});
